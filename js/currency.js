/* Roostr Guess the Currency — daily currency quiz engine. No dependencies, no
   backend. Uses the CURRENCIES dataset (inline-SVG banknotes). Five rounds a
   day, four choices each. State lives in localStorage under the "gc-" namespace. */
(function () {
  "use strict";

  /* —— configuration (mirrors js/guessflag.js) —— */
  var EPOCH = new Date(2026, 5, 1);          // day No. 1 = June 1, 2026
  var ROUNDS = 5;
  var CHOICES = 4;
  var SITE_URL = "playroostr.com/currency";  // shown in the share text
  var PAYMENT_LINK = "";
  ...
  /* Build the day's five rounds: a correct currency + three plausible
     distractors, preferring the same region, then filling from the rest.
     Offset from the flag quiz so the two rarely align on the same day. */
  function buildRounds() {
    var rng = mulberry32(DAY * 2654435761 + 733);
    var all = [];
    for (var i = 0; i < N; i++) all.push(i);
    var answers = shuffled(all, rng).slice(0, ROUNDS);

    return answers.map(function (ans) {
      var region = CURRENCIES[ans].region;
      var sameRegion = shuffled(all.filter(function (i) {
        return i !== ans && CURRENCIES[i].region === region;
      }), rng);
  ...
  function renderRound() {
    ...
    stageEl.innerHTML = CURRENCIES[r.answer].svg;

    choicesEl.className = "flag-choices";
    choicesEl.innerHTML = "";
    r.options.forEach(function (idx) {
      var btn = document.createElement("button");
      btn.type = "button";
      btn.className = "flag-choice";
      btn.dataset.cur = idx;
      btn.textContent = CURRENCIES[idx].name;
  ...
  function pick(idx) {
    ...
    btns.forEach(function (b) {
      var i = parseInt(b.dataset.cur, 10);
      if (i === r.answer) b.classList.add("flag-correct");
      else if (i === idx) b.classList.add("flag-wrong");
    });
    renderBench();
    save("gc-day", state);
  ...
  function finish() {
    ...
    if (swept()) {
      stampEl.textContent = "MONEYWISE";
      stampEl.classList.remove("lost");
      $("verdict-sub").textContent = "Five for five — you know your money! Today's currencies:";
    } else {
      stampEl.textContent = s + " / " + ROUNDS;
      stampEl.classList.add("lost");
      var msg = s === 0 ? "Tough exchange today." :
                s <= 2 ? "A few to brush up on." :
                "Solid — almost there.";
      $("verdict-sub").textContent = msg + " Today's currencies:";
    }

    var rev = $("reveal");
    rev.innerHTML = "";
    ROUND_DATA.forEach(function (r, i) {
      var c = CURRENCIES[r.answer];
      ...
      li.querySelector(".flag-mini").innerHTML = c.svg;
      li.querySelector(".flag-mini-name").innerHTML = c.name + ' <span class="cur-mini-cur">' + c.currency + '</span>';
      li.querySelector(".flag-mark").textContent = state.picks[i] ? "✓" : "✗";
      li.querySelector(".flag-mark").className = "flag-mark " + (state.picks[i] ? "ok" : "no");
      rev.appendChild(li);
    });
    $("factbox-text").textContent = CURRENCIES[ROUND_DATA[0].answer].fact;
  ...
  function shareText() {
    ...
    return "Roostr Guess the Currency #" + PUZZLE_NO + " — " + s + "/" + ROUNDS +
      (swept() ? " 💰" : " 💵") + "\n" + grid + "\n" + SITE_URL;
  }
  function share() {
    var text = shareText();
  ...
  if (state.done) {
    finish();
  } else if (!localStorage.getItem("gc-seen")) {
    save("gc-seen", 1);
